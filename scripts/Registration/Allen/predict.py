...
from scripts.Registration.Allen import configFileAllen
...
arg_parser.add_argument('--stain', help='stain used', default='MRI', choices=['NISSL', 'IHC', 'MRI']),
...
if stain_flag == 'NISSL':
    parameter_dict = configFileAllen.CONFIG_NISSL
elif stain_flag == 'IHC':
    parameter_dict = configFileAllen.CONFIG_IHC
else:
    parameter_dict = configFileAllen.CONFIG
...
try:
    ...
    print("Could not plot the results.")
###################################
########### DATA LOADER ###########
###################################
if stain_flag != 'MRI':
    data_loader = parameter_dict['DB_CONFIG']['DATA_LOADER'].DataLoader(parameter_dict['DB_CONFIG'], stain=stain_flag)
else:
...
dataset_test = datasets.IntraModalRegistrationDataset2D(
    data_loader,
    rotation_params=parameter_dict['AFFINE'],
