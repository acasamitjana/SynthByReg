...
from scripts.Registration.Cricks import configFileCricks as configFile
...
arg_parser.add_argument('--stain', help='stain used', default='MRI', choices=['DAPI', 'NEUN', 'MRI']),

arguments = arg_parser.parse_args()
stain_flag = arguments.stain

if stain_flag == 'DAPI':
    parameter_dict = configFile.CONFIG_DAPI
elif stain_flag == 'NEUN':
    parameter_dict = configFile.CONFIG_NEUN
else:
...
bidir_flag = False if stain_flag != 'MRI' else True
mask_flag = True
...
loss_function_dict = {
    ...
    'registration_smoothness': loss_smoothness(name='registration_smoothness',
                                               loss_mult=parameter_dict['UPSAMPLE_LEVELS'],
                                               **parameter_dict['LOSS_SMOOTHNESS']['params'])
}
loss_weight_dict = {
