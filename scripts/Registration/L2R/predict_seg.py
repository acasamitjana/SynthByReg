...
from src.testing import predict_segmentation
...
da_model = None#TensorDeformation(image_shape, parameter_dict['NONLINEAR'].lowres_size, device)
...
for batch_idx, data_dict in enumerate(generator_test):
    ...
    output_results = predict_segmentation(data_dict, model, device, da_model)
    ...
    for it_image, rid_image in enumerate(ref_rid_list):
        ...
        print(reg_rid)
        ref = tanh2im([output_results[0][it_image]], mask_list=[output_results[1][it_image]])[0]

        img = nib.Nifti1Image(ref, data_loader.subject_dict[ref_rid].vox2ras)
        nib.save(img, join(results_dir, 'ref_' + ref_rid + '.nii.gz'))

        img = nib.Nifti1Image(output_results[2][it_image].astype('uint16'), data_loader.subject_dict[ref_rid].vox2ras)
        nib.save(img, join(results_dir, 'ref_labels_' + ref_rid + '.nii.gz'))

        img = nib.Nifti1Image(output_results[3][it_image].astype('uint16'), data_loader.subject_dict[ref_rid].vox2ras)
        nib.save(img, join(results_dir, 'ref_seg_' + ref_rid + '.nii.gz'))

        img = nib.Nifti1Image(np.transpose(output_results[4][it_image], axes=[1,2,3,0]), data_loader.subject_dict[ref_rid].vox2ras)
        nib.save(img, join(results_dir, 'ref_seg_prob_' + ref_rid + '.nii.gz'))
