...
for batch_idx, data_dict in enumerate(generator_test):
    ...
    for it_image, rid_image in enumerate(ref_rid_list):
        ...
        img = nib.Nifti1Image(np.squeeze(output_results[-2]).astype('uint8'), data_loader.subject_dict[ref_rid].vox2ras)
        nib.save(img, join(results_dir, 'reg_ref_labels_' + ref_rid + '.nii.gz'))
