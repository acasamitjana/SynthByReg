...
arg_parser.add_argument('--block', help='stain used'),
...
if stain_flag != 'MRI':
    data_loader = DataLoader(parameter_dict['DB_CONFIG'], stain=stain_flag, bid_list=[block])
else:
    data_loader = DataLoader(parameter_dict['DB_CONFIG'], bid_list=[block])

block_loader = data_loader.subject_dict[block]
num_predictions = np.clip(num_predictions, 0, np.ceil(len(block_loader)))
