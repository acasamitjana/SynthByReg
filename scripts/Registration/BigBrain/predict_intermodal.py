...
arg_parser.add_argument('--labels', action='store_true')

arguments = arg_parser.parse_args()
epoch_weights = str(arguments.epoch)
labels_flag = arguments.labels
if labels_flag:
    parameter_dict = configFile.CONFIG_INTERMODAL_WEAKLY
else:
    parameter_dict = configFile.CONFIG_INTERMODAL
...
########### DATA LOADER ###########
###################################
data_loader = parameter_dict['DB_CONFIG']['DATA_LOADER'].DataLoader(parameter_dict['DB_CONFIG'], rid_list = ['007'])
...
print('Load model: ' + str(join(parameter_dict['RESULTS_DIR'], 'checkpoints', weightsfile)))
...
for batch_idx, data_dict in enumerate(generator_test):
    ...
    print(str(batch_idx*parameter_dict['BATCH_SIZE']) + '/' + str(int(num_predictions)) + ': ' + str(data_dict['rid']))
    ...
    for it_image, rid_image in enumerate(ref_rid_list):
        ...
        for string, frames in frames_dict.items():
            filepath = join(results_dir, ref_rid, string + '.gif')
