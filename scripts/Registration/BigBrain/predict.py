...
arg_parser.add_argument('--stain', help='stain used', default='MRI', choices=['HISTO', 'MRI']),
...
if stain_flag == 'HISTO':
    parameter_dict = configFile.CONFIG_HISTO
else:
    parameter_dict = configFile.CONFIG

parameter_dict['BATCH_SIZE'] = 1
# if parameter_dict['LOSS_REGISTRATION'] == 'NCC':
#     from src.utils.image_transforms import ScaleNormalization
#     act = 'sigmoid'
#     parameter_dict['NORMALIZATION'] = ScaleNormalization(range=[0,1], quantile=True)
# else:
#     act = 'tanh'
# parameter_dict['RESULTS_DIR'] = join(configFile. BASE_DIR, 'MRI_' + parameter_dict['LOSS_REGISTRATION'],
#                              'D' + str(parameter_dict['UPSAMPLE_LEVELS']) +
#                              '_R' + str(parameter_dict['LAMBDA_REGISTRATION']) +
#                              '_S' + str(parameter_dict['LAMBDA_SMOOTHNESS']) + '',
#                              act + '_bidir_' + str(parameter_dict['DISTANCE_NEIGHBOURS']) + 'neigh')
# parameter_dict['RESULTS_DIR'] += '_noDA' if parameter_dict['DATA_AUGMENTATION'] is None else '_DA'
...
print(parameter_dict['RESULTS_DIR'])
print(parameter_dict['UPSAMPLE_LEVELS'])
...
for batch_idx, data_dict in enumerate(generator_test):
    ...
    for it_image, rid_image in enumerate(ref_rid_list):
        ...
        slices_2d = [output_results[8][it_image, 1], output_results[8][it_image, 0]]
