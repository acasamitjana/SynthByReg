...
from src.training import InfoNCE
...
parameter_dict['WEIGHTS_REGISTRATION_MRI_TANH'] = join(parameter_dict['WEIGHTS_REGISTRATION_MRI_TANH'], block, 'checkpoints', 'model_checkpoint.FI.pth')
parameter_dict['WEIGHTS_REGISTRATION_HISTO_TANH'] = join(parameter_dict['WEIGHTS_REGISTRATION_HISTO_TANH'], block, 'checkpoints', 'model_checkpoint.FI.pth')
...
if stain_flag == 'Combined':
    checkpoint = torch.load(parameter_dict['WEIGHTS_REGISTRATION_HISTO_TANH'], map_location=device)
else:
...
# Losses
loss_function_gan = losses.DICT_LOSSES[parameter_dict['LOSS_GAN']['name']]
loss_function_reg_ncc = losses.NCC_Loss
loss_function_reg_l1 = losses.L1_Loss
loss_function_regs = losses.DICT_LOSSES[parameter_dict['LOSS_REGISTRATION_SMOOTHNESS']['name']]
loss_function_nce = []

loss_function_gan = loss_function_gan(name='gan', device=device)
loss_function_reg_ncc = loss_function_reg_ncc(name='registration_ncc', device=device, **parameter_dict['LOSS_REGISTRATION_NCC']['params'])
loss_function_reg_l1 = loss_function_reg_l1(name='registration_l1')
loss_function_regs = loss_function_regs( name='registration_smoothness', **parameter_dict['LOSS_REGISTRATION_SMOOTHNESS']['params'])
for nce_layer in nce_layers:
    loss_function_nce.append(losses.PatchNCELoss(**parameter_dict['LOSS_NCE']['params'],
...
lambda_weight_dict = {
    'gan': parameter_dict['LOSS_GAN']['lambda'],
    'nce': parameter_dict['LOSS_NCE']['lambda'],
    'registration_ncc': parameter_dict['LOSS_REGISTRATION_NCC']['lambda'],
    'registration_l1': parameter_dict['LOSS_REGISTRATION_L1']['lambda'],
...
experimentWriter.write('Number of images = ' + str(len(block_loader)))
...
training_session = InfoNCE(device, loss_function_dict, lambda_weight_dict, callback_list, da_model, nce_layers,
                           num_patches,  clip_grad, dx_lr, parameter_dict)
