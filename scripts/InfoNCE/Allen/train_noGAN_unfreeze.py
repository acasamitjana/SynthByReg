...
from src.training import InfoNCE2DNoGANJoint
from scripts.InfoNCE.Allen import configFileAllen
...
arg_parser.add_argument('--epoch_unfreeze', default=0, help='Cycle type')
...
epoch_unfreeze = int(arguments.epoch_unfreeze)

rid_list = None
configFile = configFileAllen
parameter_dict = configFile.CONFIG_UNFROZEN
...
parameter_dict['epoch_unfreeze'] = nce_layers
...
parameter_dict['LAMBDA_GAN'] = float(0)
...
parameter_dict['RESULTS_DIR'] += stain_flag + '_' + cycle +  \
                                ...
                                '_E' + str(epoch_unfreeze)

kwargs_training = {'log_interval': parameter_dict['LOG_INTERVAL'], 'weakly_flag': False, 'bidir_flag': True,
                   'epoch_unfreeze': 0}  # Number of steps
kwargs_testing = {}
...
if stain_flag == 'Combined':
    checkpoint = torch.load(parameter_dict['WEIGHTS_REGISTRATION_HISTO_SIGMOID'], map_location=device)
else:
...
optimizer_R_M = torch.optim.Adam(registration_M.parameters(), lr=0.1*gen_lr, betas=(0.5, 0.999))
optimizer_dict = {'G_M': optimizer_G_M, 'R_M': optimizer_R_M}
...
training_session = InfoNCE2DNoGANJoint(device, loss_function_dict, lambda_weight_dict, callback_list, da_model,
                                       nce_layers, num_patches,  clip_grad, dx_lr, parameter_dict,
                                       cycle=cycle, mask_nce_flag=True)
