...
parameter_dict['RESULTS_DIR'] += '_CT_' + \
...
for batch_idx, data_dict in enumerate(generator_test):
    ...
    subject = data_loader.subject_dict[data_dict['rid'][0]]
    output_results = evaluate_class.evaluate_rbs(data_dict, model_dict, device, labels_flag=True, landmarks_flag=False)
    H, seg_M, seg_H, rseg_H, landmarks = output_results
    #
    img = nib.Nifti1Image(seg_M.astype(np.uint8), subject.vox2ras)
    nib.save(img, join(results_sbj_dir, 'M_seg.nii.gz'))

    img = nib.Nifti1Image(seg_H.astype(np.uint8), subject.vox2ras)
    nib.save(img, join(results_sbj_dir, 'H_seg.nii.gz'))

    img = nib.Nifti1Image(rseg_H.astype(np.uint8), subject.vox2ras)
    nib.save(img, join(results_sbj_dir, 'H_reg_seg.nii.gz'))
    ...
    dice_0 = 0 if np.isnan(dice_0) else dice_0
    ...
    dice_2 = 0 if np.isnan(dice_2) else dice_2
    ...
    dice_3 = 0 if np.isnan(dice_3) else dice_3
    ...
    dice_4 = 0 if np.isnan(dice_4) else dice_4
...
plt.grid()
plt.legend()
plt.title('Dice 1 Init: ' + str(np.round(np.mean([m for m in total_init_dice_1 if m>0]),2)) + '. Dice 1 RegBySynth: ' + str(np.round(np.mean([m for m in total_fi_dice_1 if m>0]),2)))
...
plt.grid()
plt.legend()
plt.title('Dice 2 Init: ' + str(np.round(np.mean(total_init_dice_2),2)) + '. Dice 2 RegBySynth: ' + str(np.round(np.mean(total_fi_dice_2),2)))
...
plt.grid()
plt.legend()
plt.title('Dice 3 Init: ' + str(np.round(np.mean(total_init_dice_3),2)) + '. Dice 3 RegBySynth: ' + str(np.round(np.mean(total_fi_dice_3),2)))
plt.savefig(join(results_dir, 'dice_3.png'))

plt.figure()
plt.plot(total_init_dice_4, 'r', marker = '*', label='Initial')
plt.plot(total_fi_dice_4, 'b', marker='*', label='RegBySynth')
plt.grid()
plt.legend()
plt.title('Dice 4 Init: ' + str(np.round(np.mean(total_init_dice_4),2)) + '. Dice 4 RegBySynth: ' + str(np.round(np.mean(total_fi_dice_4),2)))
