...
from src.utils.visualization import slices, plot_results, plot_grad_flow
from src.training import InfoNCENoGAN3D
...
arg_parser.add_argument('--contrast', help='stain used', default='CT', choices=['CT'])
arg_parser.add_argument('--l_reg_l1', default=None, help='lambda for the l1 registration loss')
arg_parser.add_argument('--l_reg_labels',default=None, help='lambda for the labels registration loss')
...
l_reg_labels = arguments.l_reg_labels
...
if l_reg_labels is not None: parameter_dict['LAMBDA_REGISTRATION_LABELS'] = float(l_reg_labels)
...
if not weakly_flag:
    parameter_dict['LAMBDA_REGISTRATION_LABELS'] = 0
...
kwargs_training = {'log_interval': parameter_dict['LOG_INTERVAL'], 'weakly_flag':weakly_flag}  # Number of steps
kwargs_testing = {}
...
data_loader = db_loader.DataLoader(parameter_dict['DB_CONFIG'])

dataset_train = datasets.InterModalRegistrationDataset3D(
    data_loader,
...
model_dict = {'G_M': G_M,  'F_M': F_M, 'R_M': registration_M}
...
loss_function_reg_labels = losses.Dice_Loss
loss_function_reg_l1 = losses.L1_Loss
loss_function_regs = losses.DICT_LOSSES[parameter_dict['LOSS_REGISTRATION_SMOOTHNESS']]
loss_function_nce = []

loss_function_reg_ncc = loss_function_reg_ncc(name='registration_ncc',device=device,kernel_var=[9,9,9])
loss_function_gan = loss_function_gan(name='gan', device=device)
loss_function_reg_labels = loss_function_reg_labels(name='registration_labels')
...
loss_function_dict = {
    ...
    'registration_ncc': loss_function_reg_labels,
    'registration_labels': loss_function_reg_labels,
...
lambda_weight_dict = {
    ...
    'registration_labels': parameter_dict['LAMBDA_REGISTRATION_LABELS'],
...
optimizer_dict = {'G_M': optimizer_G_M}
...
training_session = InfoNCENoGAN3D(device, loss_function_dict, lambda_weight_dict, callback_list, da_model, nce_layers,
                           num_patches,  clip_grad, dx_lr, parameter_dict, cycle='reg')
