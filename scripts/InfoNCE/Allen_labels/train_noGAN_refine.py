...
arg_parser.add_argument('--epoch_unfreeze', default=0, help='')
arg_parser.add_argument('--weights', default=None, help='Weights to load')
...
rid_list = None
parameter_dict = configFile.CONFIG_REFINE
...
parameter_dict['RESULTS_DIR'] += 'HISTO_' + cycle + '_' + \
                                ...
                                '_E' + str(epoch_unfreeze) + '_higherLR'

parameter_dict['RESULTS_DIR'] = join(parameter_dict['RESULTS_DIR'], weights.split('/')[-3])

kwargs_training = {'log_interval': parameter_dict['LOG_INTERVAL'], 'weakly_flag': False,  'bidir_flag': False,
                   'epoch_unfreeze': 0, 'weightsfile': weights}  # Number of steps
kwargs_testing = {}
...
loss_function_reg_ncc = loss_function_reg_ncc(name='registration_ncc',device=device,kernel_var=[9, 9])
