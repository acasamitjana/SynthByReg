...
from src.utils.image_utils import tanh2im, deform2D
...
da_model = None#TensorDeformation(image_shape, nonlinear_field_size, device)
...
for it_image, (rid, subject) in enumerate(data_loader.subject_dict.items()):
    ...
    results_sbj_dir = join(results_dir, str(data_loader.rid_list[it_image]), 'grid')
    if not exists(results_sbj_dir):
        makedirs(results_sbj_dir)
    ...
    sy, sx = data_M.shape[:2]
    square_shape_y = sy // N_CTRL_POINTS_Y
    square_shape_x = sx // N_CTRL_POINTS_X

    grid = np.zeros_like(data_M,dtype='float')
    for ctrl_p_x in range(N_CTRL_POINTS_X):
        grid[:, ctrl_p_x * square_shape_x:ctrl_p_x * square_shape_x + LINE_WIDTH] = 1

    for ctrl_p_y in range(N_CTRL_POINTS_Y):
        grid[ctrl_p_y * square_shape_y:ctrl_p_y * square_shape_y + LINE_WIDTH, :] = 1

    grid[-LINE_WIDTH:] = 1
    grid[:, -LINE_WIDTH:] = 1
    grid_deformed = deform2D(grid, flow[it_image], mode='bilinear')

    #### Generated images
    grid_deformed = grid_deformed > 0.5
    # grid_deformed = 1 - grid_deformed
    output_image = np.clip(255*grid_deformed, 0, 255)
    img = Image.fromarray(output_image.astype(np.uint8), mode='L')
    img.save(join(results_sbj_dir, 'flow.png'))

    output_image = data_M
    output_image[grid > 0] = 155
    output_image = np.clip(output_image, 0, 255)
    img = Image.fromarray(output_image.astype(np.uint8), mode='L')
    img.save(join(results_sbj_dir, 'M_orig.png'))

    output_image = data_H
    output_image[grid_deformed > 0] = 155
    output_image = np.clip(output_image, 0, 255)
    img = Image.fromarray(output_image.astype(np.uint8), mode='L')
