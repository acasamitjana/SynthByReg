...
from src.training import InfoNCENoGAN
...
if db_flag != 'Allen':
    raise ValueError('Please, specify a balid DB')
...
if stain_flag == 'Combined' and 'TANH' in parameter_dict['PARENT_DIRECTORY']:
    checkpoint = torch.load(parameter_dict['WEIGHTS_REGISTRATION_HISTO_TANH'], map_location=device)

elif stain_flag == 'Combined' and 'SIGMOID' in parameter_dict['PARENT_DIRECTORY']:
    pass

elif 'TANH' in parameter_dict['PARENT_DIRECTORY']:
...
training_session = InfoNCENoGAN(device, loss_function_dict, lambda_weight_dict, callback_list, da_model, nce_layers,
