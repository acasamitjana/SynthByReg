...
class DataLoader(object):
    ...
    def _initialize_dataset(self, stain='NISSL', rid_list=None):
        self.n_dims = 2
        self.n_channels = 1
        ...
        if stain == 'Combined':
            self.database_config['MRI_DIR'] = self.database_config['NISSL_DIR']
            stain = 'IHC'

            proxy = nib.load(join(self.database_config['NISSL_DIR'], 'NISSL_images.nii.gz'))
            mri_data = proxy.get_fdata()

            proxy = nib.load(join(self.database_config['NISSL_DIR'], 'NISSL_masks.nii.gz'))
            mri_mask = proxy.get_fdata()

        else:
            proxy = nib.load(join(self.database_config['MRI_DIR'], 'MRI_images.nii.gz'))
            mri_data = proxy.get_fdata()

            proxy = nib.load(join(self.database_config['MRI_DIR'], 'MRI_masks.nii.gz'))
            mri_mask = proxy.get_fdata()

        proxy = nib.load(join(self.database_config[stain + '_DIR'], stain + '_images.nii.gz'))
        histo_data = proxy.get_fdata()

        proxy = nib.load(join(self.database_config[stain + '_DIR'], stain + '_masks.nii.gz'))
        histo_mask = proxy.get_fdata()
        with open(self.database_config[stain + '_FILE'], 'r') as csvfile:
            csvreader = csv.DictReader(csvfile)
            for row in csvreader:
                if rid_list is not None:
                    if row['tree_pos'] not in rid_list:
                        continue

                subject = Subject(id=row['tree_pos'],
                ...
                self.rid_list.append(row['tree_pos'])
                self.subject_list.append(subject)
                self.subject_dict[row['tree_pos']] = subject
